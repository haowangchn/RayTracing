...
!---------------------------------------变量声明-------------------------------------------
    ...
    real, parameter :: rfp = 2500.0
    real, parameter :: zfp = 4000.0
...
!------------------------------------------------程序计算部分-------------------------------------------------
    ...
    ! !         !! TODO 定义初值  有点问题
    ! !         write (*,*) " AOJ ", AOJ
    ! !         AOJ = 0.25 * pi * sqrt(abs(1 / sin(theta) / dtheta0))               ! 书中式子（3.52）下边的那段话 + （3.41）
    ! !         write (*,*) " AOJ ", AOJ
    ...
    ! !         ! J(k, 1) = r_a(k, 1) / sin(theta) * (r_b(k, 1) - r_a(k, 1)) / dtheta0   ! 书中式子（3.41）
    ! !         ! write (*,*) " J(k, 1) is ", J(k, 1)
            
    ! !         do i = 2, eigenray_itern
    ! !             J(k, i) = r_a(k, i) / sin(theta) * (r_b(k, i) - r_a(k, i)) / dtheta0   ! 书中式子（3.41）
    ! !             ! write (*,*) " J(k, 1) is ", J(k, 1)

    ! !             A0(k, i) = AOJ * sqrt(abs(c_c(k, i) / c0 / J(k, i)))   ! 书中式子（3.38）（3.50）
    ! !             ! write (*,*) " The A0 (", k, i, ") is", A0(k, i)


    ! !             p_auxi(k, i) = cmplx(0.0, omega * log(c_c(k, i) / c_c(k, 1)))  ! 书中式子（3.57）
    ...
    !     J(k+1, i) = r_a(k+1, i) / sin(theta) * (r_b(k+1, i) - r_a(k+1, i)) / dtheta0   ! 书中式子（3.41）
    !     A0(k+1, i) = AOJ * sqrt(abs(c_c(k+1, i) / c0 / J(k+1, i)))                   ! 书中式子（3.38）（3.50）

    !     do f = f_min, f_max, f_step ! 计算频率-压强曲线
    ...
    !         p_auxi(k+1, i) = cmplx(0.0, omega * log(c_c(k+1, i) / c_c(k+1, 1)))          ! 书中式子（3.57）
    !         p(k+1, i) = A0(k+1, i) * exp(p_auxi(k+1, i))                                 ! 书中式子（3.57）
            
    !         p_fp = 0.5 * (p(k, i) + p(k+1, i))            
    !         ! write (*, '(A)', advance='no') " f "                                    ! advance='no' 表示 不 自动换行，即光标将保持在当前行的位置
    !         write (*,*) f
    !         ! 输出文字数据
    !         ! write(*, '(A)', advance='no') "The pressure of the field of point is : "
    !         ! 输出实部和虚部
    !         write(*, '(F10.5, SP, F10.5, "i")') real(p_fp), aimag(p_fp)
